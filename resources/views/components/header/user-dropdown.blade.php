<div class="relative" x-data="{
    ...
    <button
        ...
        type="button"
    >
        <span class="mr-3 flex items-center justify-center overflow-hidden rounded-full h-11 w-11 bg-brand-100 dark:bg-brand-900/30 text-brand-600 dark:text-brand-400 font-semibold text-sm">
    ...
    <div
        ...
        class="absolute right-0 mt-[17px] flex w-[240px] flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark z-50"
        style="display: none;"
    >
        <!-- User Info -->
        <div class="pb-3 border-b border-gray-100 dark:border-gray-800">
            <span class="block font-semibold text-gray-800 text-theme-sm dark:text-white">{{ auth()->user()->name ?? 'User' }}</span>
            <span class="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400 truncate">{{ auth()->user()->email ?? '' }}</span>
            <span class="mt-1 inline-flex items-center rounded-full bg-brand-50 dark:bg-brand-950/50 px-2.5 py-0.5 text-[10px] font-bold text-brand-600 dark:text-brand-400">
        ...
        <form method="POST" action="{{ route('logout') }}"
            ...
            <button type="submit"
                ...
                @click="closeDropdown()"
            >
                <span class="text-red-500 group-hover:text-red-600 dark:text-red-400">
