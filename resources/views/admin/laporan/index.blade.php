...
<div class="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-6">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <p class="mt-1 text-xs text-gray-400">{{ \Carbon\Carbon::parse($startDate)->format('d M Y') }} – {{ \Carbon\Carbon::parse($endDate)->format('d M Y') }}</p>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        @if($layananTerlaris && $layananTerlaris->layanan)
            <h3 class="mt-1 text-xl font-bold text-gray-800 dark:text-white/90 truncate">{{ $layananTerlaris->layanan->nama_layanan }}</h3>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div class="mb-3">
            <div class="flex items-center justify-between mb-1">
                <span class="text-sm text-gray-700 dark:text-gray-300 truncate max-w-[65%]">{{ $dist->layanan->nama_layanan ?? '—' }}</span>
...
<div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] mb-6">
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        ...
        <div class="rounded-xl border border-gray-100 dark:border-gray-800 p-4 text-center">
            ...
            <p class="mt-1 text-lg font-bold text-brand-600 dark:text-brand-400">Rp {{ number_format($pb->total, 0, ',', '.') }}</p>
        </div>
        @endforeach
    </div>
</div>
        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-gray-200 dark:border-gray-800">
                    ...
                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Kode Booking</th>
                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Pelanggan</th>
                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Layanan</th>
                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Barber</th>
                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Total</th>
                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Metode</th>
                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Tanggal Bayar</th>
                </tr>
            </thead>
            <tbody>
                @forelse($transaksi as $trx)
                <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition">
                    <td class="px-5 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $loop->iteration + ($transaksi->currentPage() - 1) * $transaksi->perPage() }}</td>
                    <td class="px-5 py-4 text-sm font-mono text-gray-700 dark:text-gray-300">{{ $trx->booking->qr_code }}</td>
                    <td class="px-5 py-4 text-sm text-gray-700 dark:text-gray-300">
                        <div class="flex items-center gap-2">
                            <div class="flex h-7 w-7 items-center justify-center rounded-full bg-brand-100 dark:bg-brand-900/30 text-xs font-bold text-brand-600 dark:text-brand-400">
                                {{ strtoupper(substr($trx->booking->user->name ?? 'W', 0, 1)) }}
                            </div>
                            {{ $trx->booking->user->name ?? 'Walk-in' }}
                        </div>
                    </td>
                    <td class="px-5 py-4 text-sm text-gray-700 dark:text-gray-300">{{ $trx->booking->layanan->nama_layanan }}</td>
                    <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-400">{{ $trx->booking->barber->name }}</td>
                    <td class="px-5 py-4 text-sm font-semibold text-gray-800 dark:text-white/90">Rp {{ number_format($trx->total_harga, 0, ',', '.') }}</td>
                ...
                @empty
                <tr>
                    <td colspan="8" class="px-5 py-12 text-center text-sm text-gray-400">Tidak ada transaksi dalam periode ini.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
